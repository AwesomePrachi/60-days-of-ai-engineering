# 🚀 60 Days of Claude AI Challenge

<div align="center">

### Building AI Skills • Learning in Public • Creating Real-World Workflows

![Challenge](https://img.shields.io/badge/Challenge-60%20Days-orange?style=for-the-badge)
![AI](https://img.shields.io/badge/Focus-Claude%20AI-black?style=for-the-badge)
![Status](https://img.shields.io/badge/Status-In%20Progress-success?style=for-the-badge)

</div>

---

## 📌 About This Repository

Welcome to my **60-Day Claude AI Challenge Journey**.

Over the next **60 days**, I'm committed to learning, experimenting, and building with **Claude AI** while documenting everything publicly on GitHub.

I'm participating alongside **1000+ learners, developers, creators, and AI enthusiasts** exploring practical AI skills through this challenge.

This repository will serve as my:

* 📚 Learning Journal
* 🧠 Prompt Engineering Playground
* ⚡ AI Workflow Lab
* 🛠️ Experiment Archive
* 🚀 Public Build Log

The goal is simple:

> **Learn faster. Build smarter. Grow with AI.**

---

## 🎯 What I'm Learning

Throughout this challenge, I'll be exploring:

⚡ **AI Workflows**
⚡ **Prompt Engineering**
⚡ **Automation**
⚡ **AI Productivity**
⚡ **Context Engineering**
⚡ **Real-World AI Projects**

---

## 🎯 Goals for the Next 60 Days

* Improve practical AI skills
* Learn professional prompt engineering
* Build better AI workflows
* Increase productivity using AI tools
* Experiment with real-world use cases
* Create a strong public learning portfolio
* Build consistency through daily practice

---

## 🛠️ Tools & Stack

| Category    | Tools                                   |
| ----------- | --------------------------------------- |
| AI Tools    | Claude AI, ChatGPT                      |
| Development | VS Code, GitHub                         |
| Learning    | Prompt Engineering, AI Workflows        |
| Focus Areas | Automation, Productivity, Real-World AI |

---

## 📂 Repository Structure

```txt
60-days-of-claude-ai
│
├── README.md
│
├── Day-01
│   ├── prompt.md
│   ├── result.md
│   └── screenshot.png
│
├── Day-02
│
├── Day-03
│
└── Assets
```

---

## 📅 Daily Progress Tracker

| Day    | Topic                                       | Status      |
| ------ | ------------------------------------------- | ----------- |
| Day 01 | AI Personality Profile + Cinematic Portrait | ✅ Completed |
| Day 02 | Coming Soon                                 | ✅ Completed |
| Day 03 | Coming Soon                                 | ✅ Completed |
| Day 04 | Coming Soon                                 | ⏳           |
| Day 05 | Coming Soon                                 | ⏳           |


---

## 🌱 Why I'm Doing This

AI is changing how we learn, work, build, and create.

Instead of staying on the sidelines, I'm choosing to:

* Learn publicly
* Experiment consistently
* Build practical skills
* Grow alongside the AI community

---

## 🤝 Credits

Special thanks to **Anil Bajpai** & **ABTalksOnAI** for creating and leading the **60-Day Claude AI Mastery Challenge**.

---

## 🔗 Connect With Me

If you're also exploring AI, development, or public learning — let's connect.

**🔗 LinkedIn:** *[linkedin.com/in/awesome-prachi-patel](https://www.linkedin.com/in/awesome-prachi-patel)*

**💻 GitHub:** *[github.com/AwesomePrachi](https://www.github.com/AwesomePrachi)*

---

<div align="center">

### 🚀 Build Smarter • Learn Faster • Grow Together
